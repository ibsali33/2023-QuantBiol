...
BulkRNAseq <- read.table("Datasets/BulkRNAseq_reads.csv", header = TRUE, sep = ",")
...
# In case of location, data could refer to a city, county, state, country, etc.
# In case of anything that requires specific units of measurement
# Columns may be the result of survey questions, knowing the question is helpful
...
numbers <- c(1.1, 3.3, 5.5)
characters <- c("2", "4", "6")
words <- c("this", "is", "words")
...
testdf <- data.frame(numbers, characters, words)
...
class(BulkRNAseq$Condition)
class(wcmatches$home_score)
