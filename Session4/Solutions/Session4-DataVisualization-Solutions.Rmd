...
ggplot(SampleGenes, aes(x = MeanCounts)) + 
...
ggplot(SampleGenes, aes(MeanCounts)) + 
...
ggplot(HousekeepingGenes, aes(TimePoint, MeanCounts, color = GeneNames)) + 
 geom_line()
...
EarlyGenes <- SampleGenes %>%
 filter(Cluster == "Early")

# Add geom_point to add points to your line plot
ggplot(EarlyGenes, aes(x = TimePoint, y = MeanCounts, color = GeneNames)) + 
 geom_line() + 
 geom_point()
```
...
ggplot(SampleGenes, aes(x = TimePoint, y = MeanCounts, color = Cluster)) + 
  geom_point()+
  geom_smooth()
```
...
NormalizedData <- SampleGenes %>%
  group_by(TimePoint) %>%
  mutate(FoldChange = log2(MeanCounts/T0$MeanCounts)) %>%
  filter(Cluster == "Late")

ggplot(NormalizedData, aes(x = TimePoint, y = FoldChange, color = GeneNames)) +
  geom_line()+
...
ClusterMeans <- SampleGenes %>%
 group_by(Cluster) %>% # group the data by category of interest 
 summarize(Mean = mean(MeanCounts), StanDev = sd(MeanCounts))

ggplot(ClusterMeans, aes(x = Cluster, y = Mean, fill = Cluster, 
                          ymin = Mean-StanDev, ymax = Mean + StanDev)) + 
 geom_col() + 
 geom_errorbar(width = 0.5) 
...
ggsave(
 ...
 device = "pdf", # R will assume the correct format if it is in the name
