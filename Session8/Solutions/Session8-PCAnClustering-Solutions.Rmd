...
# Individual plot
fviz_pca_ind(YeastPCA, repel = TRUE) 

# Variable plot
fviz_pca_var(YeastPCA,
...
YeastPCAscaled <- prcomp(PCAmatrix, scale = TRUE)

# Scree plot
fviz_eig(YeastPCAscaled)
...
# Individual plot
fviz_pca_ind(YeastPCAscaled,
             repel = TRUE)

# Variable plot
fviz_pca_var(YeastPCAscaled,
             col.var = "contrib", 
             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
             repel = TRUE)
...
ProteinData <- read.csv("../Datasets/ProteinExpressionData.csv")

FlowCytMatrix <- ProteinData %>%
  select(TimePoint, FlowCytGFPSignal, GeneNames) %>%
  pivot_wider(names_from = TimePoint, values_from = FlowCytGFPSignal) %>%
  column_to_rownames("GeneNames") %>%
  as.matrix()

pheatmap(PreMatrix, # the only required argument
         scale = "none",
         ...
         main = "Heatmap of Protein Expression vs Time")
